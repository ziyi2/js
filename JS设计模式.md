...
> 需要注意的是Son类的实例对象中有name、age和names属性，Son类的原型对象中也有这些属性，只是根据原型链的搜索机制，在使用实例对象访问这些属性时，首先搜索了实例对象中的该同名属性，因此原型对象中的该属性被屏蔽。
...
| 原型模式    |  原型对象的引用类型的属性对于实例对象而言共享同一个物理空间，因此可以共享方法（方法本身也是一个引用类型），但是某些数据属性的共享容易带来副作用  |  
...
目前而言，组合继承已经可以解决大部分问题，但是也有缺陷，在实现继承时使子类的原型等于父类的实例对象，导致在子类的原型对象中还存在了父类实例对象的实例属性。寄生组合式继承可以解决在继承的过程中子类的原型对象中还存在父类实例对象的实例属性的问题。
